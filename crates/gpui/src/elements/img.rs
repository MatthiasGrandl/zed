            ...
            .paint(bounds, hitbox.as_ref(), cx, |style, cx| {
                ...
                match source {
                    ImageSource::Uri(_) | ImageSource::File(_) => {
                        let uri_or_path: UriOrPath = match self.source {
                            ImageSource::Uri(uri) => uri.clone().into(),
                            ImageSource::File(path) => path.clone().into(),
                            _ => unreachable!(),
                        };

                        cx.with_asset::<RasterOrVector>(uri_or_path, |data, cx| match data {
                            RasterOrVector::Raster(data) => {
                                let bounds = self.object_fit.get_bounds(bounds, data.size());
                                cx.paint_image(bounds, corner_radii, data, self.grayscale)
                                    .log_err();
                            }
                            RasterOrVector::Vector(data) => {
                                let scale_factor = cx.scale_factor();
                                let s = bounds.scale(scale_factor).size;
                                let key = data.to_key(size(s.width.into(), s.height.into()));
                                cx.with_asset::<Vector>(key, |data, cx| {
                                    let bounds = self.object_fit.get_bounds(bounds, data.size());
                                    cx.paint_image(bounds, corner_radii, data, self.grayscale)
                                        .log_err();
                                })
            ...
            .spawn({
                ...
                async move {
                    ...
                    let data = if let Ok(format) = image::guess_format(&bytes) {
                        ...
                        Self::Raster(ImageData::new(data))
                    } else {
